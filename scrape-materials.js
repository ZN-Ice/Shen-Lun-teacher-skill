...
function parseShenlunPaper(text) {
  ...
  // 2. 提取题目
  // 匹配 "第一题"、"第二题" 等
  const questionPattern = /第([一二三四五六七八九十]+)[题\s]*[\s\S]*?(?=第[一二三四五六七八九十]+题|$)/g;
  let questionMatch;
  let questionNum = 0;

  while ((questionMatch = questionPattern.exec(text)) !== null) {
    questionNum++;
    const questionContent = questionMatch[0].trim();
...
async function scrapeWithMaterials() {
  ...
  try {
    ...
    for (let i = 0; i < Math.min(uniqueLinks.length, limit); i++) {
      ...
      console.log(`   [${i + 1}/${Math.min(uniqueLinks.length, limit)}] 解析：${link.title}`);
      ...
      try {
        ...
        console.log(`      ✅ 解析成功，提取了 ${paper.questions.length} 道题目\n`);
